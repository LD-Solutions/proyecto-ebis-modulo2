...
const FINANCIALDATA_API_KEY: string = import.meta.env.FINANCIALDATA_API_KEY;
...
async function datosStock(simboloStock: string): Promise<StockData | null> {
  const urlOriginal: string = `${globalUrl}stock-prices?key=${FINANCIALDATA_API_KEY}&identifier=${simboloStock}`;
